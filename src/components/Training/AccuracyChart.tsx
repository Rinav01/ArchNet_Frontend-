...
export default function AccuracyChart({ metrics }: AccuracyChartProps) {
  ...
  return (
    <div className="bg-[#1e1f22]/50 border border-[#3f4046] rounded-2xl p-5 space-y-4 shadow-md select-none flex-1 min-w-[280px]">
      <div className="flex items-center gap-2 border-b border-[#3f4046]/35 pb-3">
        <div className="p-1.5 bg-[#81c784]/15 border border-[#81c784]/20 rounded-lg text-[#81c784]">
          <Activity size={14} />
        </div>
        <div>
          <h4 className="text-xs font-black text-white uppercase tracking-wider">Model Training Accuracy Curves</h4>
          <span className="text-[10px] text-gray-500 font-semibold block mt-0.5">Model fit classification metric per epoch</span>
        </div>
      </div>
        ...
        {!mounted || metrics.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%" minWidth={0}>
            <LineChart data={metrics} margin={{ top: 10, right: 10, left: -25, bottom: 5 }}>
              ...
              <YAxis stroke="#5f6368" domain={[0.4, 1.0]} />
              <Tooltip 
                contentStyle={{ 
